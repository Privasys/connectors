...
func TestNewTransportRefusesWithoutAPinnedIdentity(t *testing.T) {
	for _, c := range []struct{ host, app string }{
		...
		if _, err := NewTransport(c.host, c.app, ""); err == nil {
			t.Errorf("NewTransport(%q, %q) should have been refused", c.host, c.app)
		}
	}
	if _, err := NewTransport("Drive.Example", "CF7A0D58-5468-4168-84C3-41EBE0CE4025", ""); err != nil {
		t.Errorf("a valid pin was refused: %v", err)
	}
}

// This leg carries the holder's documents, so it must never fall back to
// plain HTTP and must never be talked into dialling somewhere else.
func TestRoundTripRefusesWrongSchemeOrHost(t *testing.T) {
	tr, err := NewTransport("drive.example", peerApp, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCheckIdentity(t *testing.T) {
	...
	t.Run("matching app id passes", func(t *testing.T) {
		tr, _ := NewTransport("drive.example", peerApp, "")
	...
	t.Run("digest pin holds and can be omitted", func(t *testing.T) {
		pinned, _ := NewTransport("drive.example", peerApp, digestHex)
		ok := withOids(
			rc.OidExtension{OID: rc.OidWorkloadAppID, Value: appBytes},
		...
		unpinned, _ := NewTransport("drive.example", peerApp, "")
