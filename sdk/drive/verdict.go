...
// (six minutes): from then on every request over that connection is refused
// with "caller presented a certificate without current evidence on this
// connection". A connection kept warm by steady traffic would otherwise
// outlive the window and turn into a permanent refusal. Idle connections are
// evicted every half window, so a fresh dial, which attests again, happens
// well inside it.
...
func roundTripStaleVerdict(rt http.RoundTripper, evict func(), req *http.Request) (*http.Response, error) {
	...
	log.Printf("[drive] %s: the peer holds no current verdict for the pooled connection; re-attesting on a fresh dial", req.URL.Host)
