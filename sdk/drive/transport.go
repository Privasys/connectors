...
// Transport is an http.RoundTripper that dials one attested peer, Drive,
// over mutual RA-TLS.
//
...
// leg carries a holder's documents. RA-TLS terminates at the peer enclave
// itself, and the peer's quote is bound to THIS handshake, so a replayed
// certificate or an intercepted session cannot pass.
//
// Deliberately narrower than the harness's transport, which fronts a whole
// tool fleet. A connector has exactly one peer, so there is no dependency
...
// NewTransport builds the transport from the runtime's environment.
//
...
// platform those are absent and the dial stays server-auth only, which is
// why the caller must still refuse to run in production without them.
func NewTransport(host, expectedAppID, expectedDigest string) (*Transport, error) {
...
func (t *Transport) RoundTrip(req *http.Request) (*http.Response, error) {
	if req.URL.Scheme != "https" {
		return nil, fmt.Errorf("attested: refusing a %s request; this leg carries the holder's documents", req.URL.Scheme)
	}
...
// byte of a document can be written to a channel whose far end has not been
...
func (t *Transport) dialVerified(_ context.Context, _ string, addr string) (net.Conn, error) {
	...
	if verr != nil {
		...
		return nil, fmt.Errorf(
			"attested: %s failed attestation, refusing to send anything: %w", host, verr)
	}
...
// NormaliseAppID accepts the forms an app id arrives in and returns 32
// lowercase hex, or "" for anything else.
