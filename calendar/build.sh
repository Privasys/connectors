...
IMAGE="${IMAGE:-ghcr.io/privasys/calendar-connector}"
TAG="${TAG:-dev}"
...
# The build context is the REPO ROOT, not this directory, so the Dockerfile
# copies `calendar/` by its path in the repository and the same file serves CI.
docker build \
  -f Dockerfile \
