...
// Command calendar-connector serves one calendar connector.
//
...
import (
	...
	"github.com/Privasys/connectors/calendar/internal/api"
...
func main() {
	...
	// forgets everything; the wallet sends back what it kept (an app
	// password the holder typed, a refresh token this service asked it to
	// hold) at the next use.
	...
	path := envOr("CALENDAR_CONFIG", "/data/calendar-connector/config.json")
	...
	go func() {
		log.Printf("calendar-connector listening on :%s", port)
...
// service that will read anyone's calendar for anyone who asks", so it is
// loud and never the default.
func requireGrant() bool {
	if os.Getenv("CALENDAR_ALLOW_UNGRANTED") == "yes-i-am-developing" {
		log.Print("DEVELOPMENT: capability checks are OFF; every caller may reach every connected calendar")
		return false
	}
	return true
}
