...
// The two-step setup: the provider first, then the button for it.
func TestSetupAsksProviderThenButton(t *testing.T) {
	...
	if w.Code != http.StatusOK || !strings.Contains(w.Body.String(), `"enum":["Microsoft","Google"]`) || strings.Contains(w.Body.String(), "x-privasys-oauth") {
		t.Fatalf("first step is the provider choice: %d %s", w.Code, w.Body)
	}
	// A mint with only the provider is one more question: the button, with
	// the provider on its start URL.
	w = r.do(http.MethodPost, "/v1/capabilities", asHolder("h1"), mintBody(map[string]any{"provider": "Google"}))
	if w.Code != http.StatusPreconditionRequired || !strings.Contains(w.Body.String(), `"provider":"Google"`) ||
		!strings.Contains(w.Body.String(), `https://files.apps.example/v1/oauth/start?kind=files.cloud`) || !strings.Contains(w.Body.String(), `provider=google`) {
		t.Fatalf("second step is the button: %d %s", w.Code, w.Body)
	}
	w = r.do(http.MethodPost, "/v1/capabilities", asHolder("h1"), mintBody(map[string]any{"provider": "microsoft"}))
	if w.Code != http.StatusPreconditionRequired || !strings.Contains(w.Body.String(), `provider=microsoft`) || !strings.Contains(w.Body.String(), `"Continue with Microsoft"`) {
		t.Fatalf("microsoft's button: %d %s", w.Code, w.Body)
	}
	// A provider nobody offers goes back to the first question.
	w = r.do(http.MethodPost, "/v1/capabilities", asHolder("h1"), mintBody(map[string]any{"provider": "Dropbox"}))
	if w.Code != http.StatusPreconditionRequired || !strings.Contains(w.Body.String(), `"enum":["Microsoft","Google"]`) {
		t.Fatalf("unknown provider: %d %s", w.Code, w.Body)
	}
	// A provider without a client configured says so.
	r.s.applyConfig(clients(false, true))
	w = r.do(http.MethodPost, "/v1/capabilities", asHolder("h1"), mintBody(map[string]any{"provider": "Microsoft"}))
	if w.Code != http.StatusServiceUnavailable || !strings.Contains(w.Body.String(), "no Microsoft OAuth client") {
		t.Fatalf("unconfigured provider: %d %s", w.Code, w.Body)
	}
}

// The whole Microsoft path: sign in, mint with the grant, the credential is
// proved and kept, the wallet keeps the refresh token; a tool call runs
// with the bearer and refreshes it with the scope named; then a restart,
// the wallet sends the token back, and the account is connected without a
// browser.
func TestMicrosoftSignInMintAndKeptToken(t *testing.T) {
	...
	w := r.do(http.MethodPost, "/v1/capabilities", asHolder("m"), mintBody(map[string]any{"provider": "Microsoft", "grant": grant}))
	...
	decode(t, w, &got)
	if got.Keep["refresh_token"] != "rt-1" || got.ServiceResult["account"] != "me@example.org" {
		t.Fatalf("the wallet keeps the refresh token and learns the account: %s", w.Body)
	}
	...
	if w := r.do(http.MethodPost, "/v1/capabilities", asHolder("m2"), mintBody(map[string]any{"provider": "Microsoft", "grant": grant})); w.Code != http.StatusPreconditionRequired || !strings.Contains(w.Body.String(), "already used") {
	...
	r2 := oauthRig(t, ms, gg)
	w = r2.do(http.MethodPost, "/v1/capabilities", asHolder("m"), mintBody(map[string]any{"provider": "Microsoft", "kept": map[string]any{"refresh_token": "rt-1"}}))
	...
	w = r2.do(http.MethodPost, "/v1/capabilities", asHolder("m3"), mintBody(map[string]any{"provider": "Microsoft", "kept": map[string]any{"refresh_token": "rt-revoked"}}))
...
// The Google path goes through the sdk's refresh, with no scope.
func TestGoogleSignInAndRefresh(t *testing.T) {
	...
	w := r.do(http.MethodPost, "/v1/capabilities", asHolder("g"), mintBody(map[string]any{"provider": "Google", "user": "ME@example.org", "grant": grant}))
	...
	if acct.Provider != cloud.ProviderGoogle || acct.User != "me@example.org" {
	...
	// The sign-in must be for the address the holder typed, when they did.
	r.identity = "other@example.org"
	grant = signIn(t, r, gg, "google")
	w = r.do(http.MethodPost, "/v1/capabilities", asHolder("g2"), mintBody(map[string]any{"provider": "Google", "user": "me@example.org", "grant": grant}))
	if w.Code != http.StatusBadRequest || !strings.Contains(w.Body.String(), "other@example.org") {
