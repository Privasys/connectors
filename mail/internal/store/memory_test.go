...
func TestMemoryKeepsOneAccountPerSubject(t *testing.T) {
	m := NewMemory()
	ctx := context.Background()

	if _, err := m.Get(ctx, "user-1"); !errors.Is(err, ErrNoAccount) {
		t.Fatalf("an unknown subject is ErrNoAccount, got %v", err)
	}
	if err := m.Put(ctx, "user-1", Account{Provider: "imap", User: "a@example.com", Secret: "pw-1"}); err != nil {
		t.Fatal(err)
	}
	if err := m.Put(ctx, "user-2", Account{Provider: "imap", User: "b@example.com", Secret: "pw-2"}); err != nil {
		t.Fatal(err)
	}
	...
	// Connecting again replaces: there is one mailbox per holder here.
	_ = m.Put(ctx, "user-1", Account{Provider: "imap", User: "c@example.com", Secret: "pw-3"})
...
// the test pins it, so nobody adds a file behind it without noticing.
func TestMemoryCloseForgetsEverything(t *testing.T) {
	m := NewMemory()
	ctx := context.Background()
